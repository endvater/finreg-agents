...
class RegulatoryParser:
    ...
    def __init__(
        self, fallback_chunk_size: int = 1000, fallback_chunk_overlap: int = 100
    ):
        self.fallback_chunk_size = fallback_chunk_size
        self._fallback_splitter = SentenceSplitter(
            chunk_size=fallback_chunk_size, chunk_overlap=fallback_chunk_overlap
        )
        ...
        self.patterns = {
            "chapter": re.compile(
                r"^(?:Kapitel|Teil|Abschnitt)\s+(?:[IVX]+|\d+)(?=\s*[:.\s])",
                re.MULTILINE | re.IGNORECASE,
            ),
            "module": re.compile(
                r"^(?:Modul\s+)?(AT|BTR|BTO|BTK)\s+(\d+(?:\.\d+)*)\s*", re.MULTILINE
            ),
            "article": re.compile(
                r"^(?:Art\.|Artikel|Article|Recital)\s+(\d+[a-z]*)",
                re.MULTILINE | re.IGNORECASE,
            ),
            "paragraph": re.compile(r"^§\s*(\d+[a-z]*)", re.MULTILINE),
            "margin_no": re.compile(
                r"^(?:Tz\.|Textziffer)\s*(\d+)", re.MULTILINE | re.IGNORECASE
            ),
            "sub_paragraph": re.compile(
                r"^\s*\(\s*(\d+[a-z]*)\s*\)(?=\s+\w{3,})", re.MULTILINE
            ),
        }

    def parse_text(
        self, text: str, base_metadata: Dict[str, Any] = None
    ) -> List[TextNode]:
        """
    ...
    def _create_nodes_from_markers(
        self, text: str, markers: List[Dict], base_metadata: Dict
    ) -> List[TextNode]:
        ...
        for i, marker in enumerate(markers):
            ...
            end_idx = (
                int(markers[i + 1]["start"]) if i + 1 < len(markers) else len(text)
            )
            ...
            if context_state["article"]:
                if "Recital" in str(
                    context_state["article"]
                ) or "Recital" in chunk_meta.get("structural_id", ""):
