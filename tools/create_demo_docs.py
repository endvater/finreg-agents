...
try:
    ...
    sys.exit(
        "pandas / openpyxl fehlt. Bitte installieren: pip install pandas openpyxl"
    )
...
def create_risikoanalyse(out: Path):
    ...
    cols = [("Segment", 50), ("Anzahl Kunden", 35), ("Risikoklasse", 35), ("Begründung", 50)]
    ...
    rows = [
        ...
        ("PEPs und Familienangehörige", "    82", "Sehr hoch",  "§15 GwG verstärkte Pflichten"),
...
def create_tm_konzept(out: Path):
    ...
    _body(pdf,
        "Das TM-System arbeitet mit 12 aktiven Monitoring-Szenarien:"
    )
    cols = [("Szenario-ID", 25), ("Bezeichnung", 70), ("Schwellenwert", 40), ("Risikoklasse", 35)]
    _table_header(pdf, cols)
    szenarien = [
...
def create_sar_verfahren(out: Path):
    ...
    _label_value(pdf, "Gilt für", "GwB-Team, Compliance, alle Mitarbeiter mit Meldeverdacht")
    pdf.ln(4)
...
def create_schulungskonzept(out: Path):
    ...
    _label_value(pdf, "Zielgruppen", "Alle Mitarbeiter (Pflichtschulung); vertieft für Vertrieb, Compliance, IT")
...
def create_gwb_jahresbericht(out: Path):
    ...
    _label_value(pdf, "Genehmigt", "Gesamtvorstand, 28.01.2025 (TOP 3 Vorstandssitzung)")
    pdf.ln(4)
    ...
    for i, row in enumerate(kpis):
        _table_row(pdf, row, widths, alt=bool(i % 2))
    pdf.ln(2)
    _body(pdf, "* Anstieg durch krankheitsbedingte Personalausfälle im Compliance-Team (Q3-Q4 2024).")
...
def create_tm_alerts_excel(out: Path):
    ...
    szenarien_names = [
        "Grosse Bareinzahlung", "Strukturierung (Smurfing)", "Auslandsüberweisung",
        "Risikostaat-Transaktion", "Schnelle Kontenentleerung", "Neukundenaktivität",
        "PEP-Transaktion", "Round-Tripping", "Ungewöhnliche Frequenz",
        "Dormant Account", "Correspondent Banking", "Krypto-Bezug",
    ]
    ...
    df_szenarien = pd.DataFrame({
        ...
        "Konversionsrate_SAR_Prozent": [round(s / a * 100, 2) for s, a in
                                         zip(sar_count, total_alerts_szenario)],
    })
...
def create_schulungsmatrix_excel(out: Path):
    ...
    # Gesamtzeile
    total = df[["Mitarbeiter_gesamt", "Abgeschlossen", "Ausstehend_gesamt",
                "Ausstehend_Krankheit", "Ausstehend_Neuzugang",
                "Ausstehend_ohne_Grund"]].sum()
    total_row = {
        "Abteilung": "GESAMT", "Kurstyp": "",
        ...
        "Quote_Prozent": round(total["Abgeschlossen"] / total["Mitarbeiter_gesamt"] * 100, 1),
    }
...
def create_sar_statistik_csv(out: Path):
    ...
    for _ in range(89):
        ...
        szenario = rng.choice([
            "Strukturierung", "Bareinzahlung > 9.000 EUR", "Auslandsüberweisung",
            "PEP-Transaktion", "Kontoverhalten auffällig", "Mitarbeitermeldung",
        ])
        rows.append({
...
def create_tm_system_log(out: Path):
    ...
    event_types = [
        ...
        ("ALERT", "Neuer Alert generiert: {id} | Szenario: {szenario} | Konto: XXXX{kto}"),
...
def create_goaml_log(out: Path):
    ...
    lines += [
        "",
        f"# ZUSAMMENFASSUNG: 89 SARs eingereicht | Durchschn. Bearbeitungszeit: 3.2 Tage",
        f"# Alle SARs innerhalb der gesetzlichen Frist (§ 43 GwG) eingereicht: JA",
    ]
    out.write_text("\n".join(lines) + "\n", encoding="utf-8")
    log.info("  ✓ %s", out.name)
...
def generate_all(output_dir: str):
    ...
    create_interview_compliance(dirs["interviews"] / "interview_compliance_officer.json")
    ...
    log.info("  ✅ KONFORM:        S04-01, S05-01, S05-02, S05-03, S02-01, S02-03, S02-04")
    log.info("  ⚠️  TEILKONFORM:   S01-01, S01-03, S02-02, S02-06, S03-02, S04-02, S06-01, S06-02")
    ...
    log.info("  python pipeline.py --input %s --institution \"Musterbank AG\" "
             "--regulatorik gwg --skeptiker", base.resolve())
...
def main():
    ...
    parser.add_argument(
        "--output", default="./demo",
        help="Ausgabeverzeichnis (default: ./demo)"
    )
